...
func (r *postgresRepository) CreateUser(ctx context.Context, tx pgx.Tx, u *User) (uuid.UUID, error) {
	const query = `
		INSERT INTO users (email, password_hash, email_verified, status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id
	`
	var id uuid.UUID
	err := tx.QueryRow(ctx, query,
		u.Email, u.PasswordHash, u.EmailVerified, u.Status, u.CreatedAt, u.UpdatedAt,
	).Scan(&id)
	return id, err
}
...
func (r *postgresRepository) GetByID(ctx context.Context, id uuid.UUID) (*User, error) {
	...
	`, id).Scan(&u.ID, &u.Email, &u.PasswordHash, &u.EmailVerified, &u.Status, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &u, nil
...
func (r *postgresRepository) GetByEmail(ctx context.Context, email string) (*User, error) {
	...
	`, email).Scan(&u.ID, &u.Email, &u.PasswordHash, &u.EmailVerified, &u.Status, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &u, nil
...
func (r *postgresRepository) GetVerificationByToken(ctx context.Context, token string) (*EmailVerification, error) {
	...
	`, token).Scan(&ev.ID, &ev.UserID, &ev.Token, &ev.ExpiresAt, &ev.VerifiedAt, &ev.CreatedAt)
	return &ev, err
}

func (r *postgresRepository) MarkEmailVerified(ctx context.Context, userID uuid.UUID) error {
	cmd, err := r.db.Pool.Exec(ctx,
		`UPDATE users SET email_verified = TRUE WHERE id = $1 AND email_verified = FALSE`, userID)
	if err != nil {
		return err
	}
	...
	_, err = r.db.Pool.Exec(ctx,
		`UPDATE email_verifications SET verified_at = NOW() WHERE user_id = $1 AND verified_at IS NULL`, userID)
	return err
}
